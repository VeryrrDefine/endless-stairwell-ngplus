// Code snippets from format-expantanum.js
var PowiainaNum;
let MAX_LOGP1_REPEATS = 48
let LOG5E = 0.6213349345596119
if (!PowiainaNum){
    PowiainaNum = require("./PowiainaNum");
}
...
function polarize(array, smallTop=false) {
    ...
    */
    ...
    let searchMax = Math.max(array.findIndex(function(x){return x[0]=="x"}), 1)
    ...
    if (!Number.isFinite(bottom)) {}
        ...
        height = array[elem][0]
        var repeatCountLess = 500
        while (repeatCountLess > 0 && (bottom >= 10 || elem < searchMax || (smallTop && top >= 10))) {
            if (bottom >= 10) { // Bottom mode: the bottom number "climbs" to the top
                ...
                if (elem == searchMax-1 && array[elem][0] == height && !(smallTop && top >= 10)) break
                
                bottom = Math.log10(bottom) + top
                height += 1
                if (elem < searchMax && height > array[elem][0]) elem += 1
                if (elem < searchMax) {
...
let a = PowiainaNum("J^4 (10{10})^9 (10{9})^9 (10{8})^9 (10{7})^9 (10{6})^9 (10{5})^9 (10{4})^9 (10{3})^9 (10{2})^9 (10{1})^9 297121486764.58594").normalize()
