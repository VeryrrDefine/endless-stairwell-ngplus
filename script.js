var styleTag = document.querySelector("style#styleTag")
function reset() {
  game = {
    currentFloor: 0,
    floorsWithRooms: [[], [], [], [], [], [], [], [], [], []],
...
function save() {
  if (game.fightingMonster == false) { localStorage.setItem("endlessStairwellSave1p2p0", JSON.stringify(game)) }
}
...
function load() {
  reset()
  let loadgame = JSON.parse(localStorage.getItem("endlessStairwellSave1p2p0"))
  if (loadgame != null) {
    loadGame(loadgame)
  }
}
...
function loadGame(loadgame) {
  ...
  if (!game.starPieces) game.starPieces = 0;
  ...
  if (game.floorsWithRooms.length < 9) {
    game.floorsWithRooms[8] = []
    if (game.floorsWithRooms[8].length == 0) {
      for (i = 0; i < 4; i++) {
        x = Math.floor(Math.random() * 39 + 501)
        if (!game.floorsWithRooms[8].includes(x)) { game.floorsWithRooms[8].push(x) }
        else if (!game.floorsWithRooms[8].includes(x - 1)) { game.floorsWithRooms[8].push(x - 1) }
        else if (!game.floorsWithRooms[8].includes(x - 2)) { game.floorsWithRooms[8].push(x - 2) }
        else { game.floorsWithRooms[8].push(x - 3) }
      }
    }
  ...
  if (game.floorsWithRooms.length < 10){
    ...
    if (game.floorsWithRooms[9].length == 0) {
      for (i = 0; i < 4; i++) {
        x = Math.floor(Math.random() * 39 + 551)
        if (!game.floorsWithRooms[9].includes(x)) { game.floorsWithRooms[9].push(x) }
        else if (!game.floorsWithRooms[9].includes(x - 1)) { game.floorsWithRooms[9].push(x - 1) }
        else if (!game.floorsWithRooms[9].includes(x - 2)) { game.floorsWithRooms[9].push(x - 2) }
        else { game.floorsWithRooms[9].push(x - 3) }
      }
    }
    game.floorsWithRooms[9] = game.floorsWithRooms[9].sort(function (a, b) { return a - b })
  }
  ...
  document.getElementById("toFloor500Button").style.display = "none"
  $("#roomInfo").html("")
  ...
  if (game.roomsExplored > 0 || game.monstersKilled > 0 || game.honey > 0 || game.vanillaHoney > 0 || game.cocoaHoney.gt(0) || game.darkOrbs > 0 || game.starPieces > 0) {
  ...
  if (game.goldenEelDefeated && game.roomsExplored == 0 && isFloorAllowed(500)) {
    document.getElementById("toFloor500Button").style.display = "block"
  }
  ...
  for (i = 0; i < game.goldenUpgrades2Bought.length; i++) {
  ...
  if (typeof game.goldenHoney == "number") game.goldenHoney = PowiainaNum.fromNumber(game.goldenHoney)
}
...
function getTheTime() {
  ...
  theTime = `${d.getFullYear().toString()}-${(d.getMonth() + 1).toString().padStart(2, "0")}-${(d.getDate()).toString().padStart(2, "0")} ${d.getHours().toString().padStart(2, "0")}:${d.getMinutes().toString().padStart(2, "0")}:${(d.getSeconds()).toString().padStart(2, "0")}`
...
function updateSmall() {
  ...
  if (game.goldenHoney.lte(0)) { game.attackDamage = PowiainaNum(10).mul(PowiainaNum(1.1).pow(game.level.sub(1))).floor() }
  else { game.attackDamage = PowiainaNum.expansion(11, attackExpansionlog()) }
  ...
  $("#starPieces").html(game.starPieces)
  $("#starPieceRequirement").html(format(starPieceRequirements[game.starPieces]))
...
function updateInfo() {
  if (game.roomsExplored == 0) {
    ...
    else if (game.currentFloor == 500) { $("#info").html("<b>Info</b><br>You are in a seemingly endless stairwell.") }
    else if (game.currentFloor == -2) { $("#info").html("<b>Info</b><br>You are in a seemingly endless stairwell.") }
    ...
    else if (game.floorsWithRooms[0].includes(game.currentFloor) || game.floorsWithRooms[1].includes(game.currentFloor) || game.floorsWithRooms[2].includes(game.currentFloor) || game.floorsWithRooms[3].includes(game.currentFloor) || game.floorsWithRooms[4].includes(game.currentFloor) || game.floorsWithRooms[5].includes(game.currentFloor) || game.floorsWithRooms[6].includes(game.currentFloor) || game.floorsWithRooms[7].includes(game.currentFloor) || game.floorsWithRooms[8].includes(game.currentFloor) || game.floorsWithRooms[9].includes(game.currentFloor)) {
      document.getElementById("enterFloorButton").disabled = false
    ...
    if (game.currentFloor == -2) { document.getElementById("starResetDiv").style.display = "block" }
    else { document.getElementById("starResetDiv").style.display = "none" }
  }
}
...
function toStairwell() {
  if (game.roomsFromStairwell <= 1) {
    ...
    if (game.goldenEelDefeated && isFloorAllowed(500)) {
      document.getElementById("toFloor500Button").style.display = "block"
    }
  }
  else {
...
function monsterEncounter() {
  ...
  else if (game.currentFloor <= 599) { monsterType = Math.max(Math.ceil(Math.random() * 6) + 37, 38) }
  ...
  else if (game.currentFloor > 304) { game.monsterMaxHealth = PowiainaNum(PowiainaNum.hyper(monsters[monsterType - 1].health.mul(game.totalDifficulty.sub(0.6)).sub(2).floor())(10, (Math.random() * 8 + 2))) }
  ...
  //Monster damage
  //if (game.currentFloor < -1) { $("#monsterDamage").html(format(game.monsterMaxHealth), 0) }
  if (game.currentFloor > 100) { $("#monsterDamage").html(format(game.monsterMaxHealth), 0) }
  else if (game.currentFloor > 50 || game.currentFloor < -1) { $("#monsterDamage").html(format((monsters[monsterType - 1].damage.mul(PowiainaNum(1.2).pow(game.totalDifficulty.mul(3).sub(1))).floor()), 0)) }
...
function randomItem() {
  if (Math.floor(Math.random() * 100) >= 30) {
    ...
    if (game.currentFloor == 49 && game.specialItemsAcquired[1] != true) {
      ...
      if (game.vanillaHoney.gte(1) && game.starPieces >= 1) game.vanillaHoney = game.vanillaHoney.tetrate(2)
  ...
  if (game.fightingMonster == true) {
    if (game.monsterAttackCooldown >= 0.1) {
      ...
      if (game.currentFloor == 499) {
        ...
        if (game.currentFloor > 100 || game.currentFloor < -1) { game.health = game.health.sub(game.monsterMaxHealth).floor() }
...
function toFloor299() {
  if (game.specialItemsAcquired[1] == true && isFloorAllowed(299)) {
...
function toFloor500() {
  if (isFloorAllowed(500)) {
    game.currentFloor = 500
    $("#currentFloor").html("floor " + game.currentFloor)
    document.getElementById("enterFloorButton").style.display = "none"
    document.getElementById("floorDownButton").disabled = false
    document.getElementById("floorUpButton").disabled = false
...
function buyCombinatorUpgrade2(x) {
  ...
  else if (x == 11 && game.cocoaHoney.gte("J1e15") && game.darkOrbs >= 4 && game.combinatorUpgrades2Bought[10] != true) {
...
function buyGoldenUpgrade2(x) {
  if (x == 1 && game.goldenHoney.gte(1e13) && game.goldenUpgrades2Bought[0] != true) {
    ...
    document.getElementsByClassName("goldenUpgrade2")[0].disabled = true
  }
  if (x == 2 && game.goldenHoney.gte(1.5e13) && game.goldenUpgrades2Bought[1] != true) {
    game.goldenUpgrades2Bought[1] = true
    document.getElementsByClassName("goldenUpgrade2")[1].disabled = true
  }
  if (x == 3 && game.goldenHoney.gte(5e14) && game.goldenUpgrades2Bought[2] != true) {
    game.goldenUpgrades2Bought[2] = true
    document.getElementsByClassName("goldenUpgrade2")[2].disabled = true
  }
  if (x == 4 && game.goldenHoney.gte(6.5e14) && game.goldenUpgrades2Bought[3] != true) {
    game.goldenUpgrades2Bought[3] = true
    document.getElementsByClassName("goldenUpgrade2")[3].disabled = true
  }
  if (x == 5 && game.goldenHoney.gte(1e15) && game.goldenUpgrades2Bought[4] != true) {
    game.goldenUpgrades2Bought[4] = true
    document.getElementsByClassName("goldenUpgrade2")[4].disabled = true
  }
  if (x == 6 && game.goldenHoney.gte(1.5e15) && game.goldenUpgrades2Bought[5] != true) {
  ...
  if (x == 12 && game.goldenHoney.gte("ee3.4028236e38") && game.goldenUpgrades2Bought[11] != true) {
...
function goldenEelAttack() {
  if (game.energy > 0) {
    ...
    game.timeSinceAttack = 1
    game.goldenEelHealth -= game.goldenHoney.toNumber()
...
function starPiecePrestige() {
  if (game.goldenHoney.gte(starPieceRequirements[game.starPieces])) {
    if (confirm("Are you sure you want to reset EVERYTHING for a STAR PIECE?")) {
      game.starPieces++
      starPieceReset()
      $("#starPieceBonuses").html(starPieceBonuses[game.starPieces])
...
function starPieceReset(){
  ...
  game.darkBars = 0
  game.starBars = 0
  game.specialItemsAcquired = [true, false]
  document.getElementsByClassName("combinatorButton")[0].disabled = true
  document.getElementsByClassName("combinatorText")[0].innerHTML = "Hyperplasm (not unlocked)"
  document.getElementsByClassName("combinatorButton")[1].disabled = true
  document.getElementsByClassName("combinatorText")[1].innerHTML = "Dark bar (not unlocked)"
